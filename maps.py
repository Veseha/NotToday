...
import pygame
import os
from global_varibals import MAXENERGY, FPS, STEPS
...
def start_screen():  # ????????? ????, ????? ???????????
    intro_text = ['Это игра сделана рукожопом, 5xAlpha test']
...
class Kostyl(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        # self.stand = pygame.transform.scale(load_im3age('player/stand.png', -1), (sx, sy))
